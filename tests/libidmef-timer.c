...
static void timer_callback(void *data)
{
        ...
        assert(elapsed == libidmef_timer_get_expire(&timer->timer));

        libidmef_timer_destroy(&timer->timer);
        free(timer);
...
int main(int argc, char **argv)
{
        ...
        while ( i-- ) {
                ...
                libidmef_timer_set_callback(&timer->timer, timer_callback);
                libidmef_timer_set_data(&timer->timer, timer);
                libidmef_timer_set_expire(&timer->timer, expire);
                libidmef_timer_init(&timer->timer);
