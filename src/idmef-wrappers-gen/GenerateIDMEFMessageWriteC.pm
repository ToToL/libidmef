...
sub        struct_field_normal
{
    ...
    if ( $field->{metatype} & &METATYPE_OPTIONAL_INT ) {
        $self->output("
        {
                ...
                if ( tmp ) \{
                        ret = $function(*tmp, fd, IDMEF_MSG_",  uc($struct->{short_typename}), "_", uc($field->{name}), ");
                        if ( ret < 0 )
                                return ret;");
        ...
        $self->output(" " x 8,
                      ...
                      "fd, IDMEF_MSG_",  uc($struct->{short_typename}), "_", uc($field->{name}),
                      ");\n",
                      " " x 8, "if ( ret < 0 )\n",
                      " " x 16, "return ret;\n\n");
...
sub        struct_field_struct
{
    ...
    $self->output(" " x 8,
                  "ret = idmef_$field->{short_typename}_write(idmef_$struct->{short_typename}_get_$field->{name}($struct->{short_typename}), fd);\n",
                  " " x 8, "if ( ret < 0 )\n", " " x 16, "return ret;\n");
}
...
sub        struct_field_list
{
    ...
    if ( $field->{metatype} & &METATYPE_PRIMITIVE ) {
        ...
        $self->output(" " x 24, "ret = idmef_$field->{short_typename}_write($field->{short_name}, fd);\n",
                      " " x 24, "if ( ret < 0 )\n", " " x 32, "return ret;\n");
    }
...
sub        struct_field_union
{
    ...
    foreach my $member ( @{$field->{member_list}} ) {
        ...
        $self->output(" " x 24, "ret = idmef_$member->{short_typename}_write(idmef_$struct->{short_typename}_get_$member->{name}($struct->{short_typename}), fd);", "\n");
...
int idmef_$struct->{short_typename}_write($struct->{typename} *$struct->{short_typename}, prelude_io_t *fd)\n\{\n", " " x 8, "int ret;\n");
    ...
    if ( ! $struct->{toplevel} ) {
        $self->output(" " x 8, "ret = binary_write(fd, ", "IDMEF_MSG_" . uc($struct->{short_typename}) . "_TAG", ", 0, NULL);\n",
" " x 8, "if ( ret < 0 )\n", " " x 16, "return ret;\n");
    }
    ...
    $self->output(" " x 8, "return binary_write(fd, IDMEF_MSG_END_OF_TAG, 0, NULL);\n");
    $self->output("\}\n\n\n");
}
