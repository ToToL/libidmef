...
");
    ...
    foreach my $field ( map { $_->{metatype} & &METATYPE_LIST ? $_ : () } @{ $struct->{field_list} } ) {
        ...
        $self->output("
        libidmef_list_init(&(*ret)->$field->{name});
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( $field->{typename} ne "libidmef_string_t" and !($field->{metatype} & (&METATYPE_PRIMITIVE)) ) {
                $prefix = "idmef_";
        }
...
");
                ...
                #$self->output("                int retval = libidmef_string_new_ref_fast(&(*ret)->$field->{name}, \"1.0\", 3);");
        #} else {
...
");
            ...
            if ( $field->{typename} eq "libidmef_string_t" ) {
                my $refer = $field->{ptr} ? "" : "&";
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        if ( $field->{metatype} & &METATYPE_LIST ) {
                ...
                if ( $field->{typename} eq "idmef_time_t" ||
                     ...
                     $field->{typename} eq "libidmef_string_t" ) {
            ...
            $self->output("
                case $n: \{
                        ...
                        if ( n >= 0 ) {
                               libidmef_list_for_each(&ptr->$field->{name}, tmp) {
                                       if ( i++ == n ) {
                               ...
                               libidmef_list_for_each_reversed(&ptr->$field->{name}, tmp) {
                                       if ( i++ == pos ) {
                                               void *b = libidmef_linked_object_get_object(tmp);
                                               ${type}_destroy(b);
                                               return 0;
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                libidmef_list_for_each_safe(&dst->$field->{name}, tmp, n) \{
                        entry = libidmef_linked_object_get_object(tmp);
                        $destroy_func(entry);
                \}

                libidmef_list_for_each_safe(&src->$field->{name}, tmp, n) \{
                        entry = libidmef_linked_object_get_object(tmp);
                        $clone_func(entry, &new);
                        libidmef_list_add_tail(&dst->$field->{name}, &((libidmef_linked_object_t *) new)->_list);
                \}
        \}
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                do \{
                        ...
                        libidmef_list_for_each_continue(&obj1->$field->{name}, tmp1) \{
                                entry1 = libidmef_linked_object_get_object(tmp1);
                                break;
                        \}

                        libidmef_list_for_each_continue(&obj2->$field->{name}, tmp2) \{
...
");
    ...
    foreach my $field ( @{ $struct->{field_list} } ) {
        ...
        \{
                ...
                libidmef_list_for_each_safe(&ptr->$field->{name}, tmp, n) \{
                        entry = libidmef_linked_object_get_object(tmp);
...
\{
        libidmef_return_val_if_fail($struct->{short_typename}, NULL);
...
\{
        ...
        libidmef_return_if_fail(ptr->$field->{name});
...
");
                ...
                if ( $tree->{objs}->{$field->{typename}}->{is_listed} ) {
                     ...
                     $self->output("
        libidmef_return_val_if_fail(ptr, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_list_init(&ptr->$field->{name}._list);");
                 }
            }
        }
    }
...
\{
        libidmef_list_t *tmp = ($field->{short_typename}_cur) ? &((libidmef_linked_object_t *) $field->{short_typename}_cur)->_list : NULL;

        libidmef_return_val_if_fail($struct->{short_typename}, NULL);

        libidmef_list_for_each_continue(&$struct->{short_typename}->$field->{name}, tmp)
...
\{
        ...
        list_insert(&ptr->$field->{name}, &((libidmef_linked_object_t *) object)->_list, pos);
\}
...
\{
        ...
        list_insert(&ptr->$field->{name}, &((libidmef_linked_object_t *)(*ret))->_list, pos);
...
\{
   ...
   $self->output("
        ...
        return libidmef_error_verbose(LIBIDMEF_ERROR_IDMEF_UNKNOWN_ENUM_STRING, \"Unknown enumeration value '%s' for $enum->{short_typename}\", name);
\}
");
