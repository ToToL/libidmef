...
");
        ...
        $self->output("
                ...
                \{
                        print_indent(fd);
                        libidmef_io_write(fd, \"$field->{name}: \", sizeof(\"$field->{name}: \") - 1);
...
");
            ...
            $self->output("                        libidmef_io_write(fd, \"\\n\", sizeof(\"\\n\") - 1);
                \}
        \}
");
...
");
        ...
        \{
                ...
                if ( field ) \{
                        print_indent(fd);
                        libidmef_io_write(fd, \"$field->{name}:\\n\", sizeof(\"$field->{name}:\\n\") - 1);
...
sub        struct_field_union
{
    ...
    foreach my $member ( @{ $field->{member_list} } ) {
        ...
        case $member->{value}:
                print_indent(fd);
                libidmef_io_write(fd, \"$member->{name}:\\n\", sizeof(\"$member->{name}:\\n\") - 1);
 ...
 */
void idmef_$struct->{short_typename}_print($struct->{typename} *ptr, libidmef_io_t *fd)
\{
        if ( ! ptr )
                return;
