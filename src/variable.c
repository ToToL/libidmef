...
static LIBIDMEF_LIST(variable_list);
...
static variable_t *search_entry(const char *variable)
{
        ...
        libidmef_list_for_each(&variable_list, tmp) {
                item = libidmef_list_entry(tmp, variable_t, list);
...
static int create_entry(const char *variable, const char *value)
{
        ...
        libidmef_list_add_tail(&variable_list, &item->list);
...
{
        variable_t *item;
        libidmef_list_t *tmp, *bkp;

        libidmef_list_for_each_safe(&variable_list, tmp, bkp) {
                item = libidmef_list_entry(tmp, variable_t, list);
                destroy_variable(item);
        }
}
