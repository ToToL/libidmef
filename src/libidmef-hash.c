...
struct libidmef_hash {
        size_t lists_size;
        libidmef_list_t *lists;
...
static unsigned int hash_value_calc(libidmef_hash_t *hash, const void *key)
{
...
static hash_elem_t *hash_elem_get(libidmef_hash_t *hash, const void *key)
{
        libidmef_list_t *list;
        libidmef_list_t *ptr;
        ...
        libidmef_list_for_each(list, ptr) {
                hash_elem = libidmef_list_entry(ptr, hash_elem_t, list);
...
static  void hash_elem_key_destroy(libidmef_hash_t *hash, hash_elem_t *hash_elem)
{
...
static void hash_elem_value_destroy(libidmef_hash_t *hash, hash_elem_t *hash_elem)
{
...
int libidmef_hash_new(libidmef_hash_t **nhash,
...
{
        return libidmef_hash_new2(nhash, HASH_DEFAULT_SIZE, hash_func, key_cmp_func, key_destroy_func, value_destroy_func);
}



int libidmef_hash_new2(libidmef_hash_t **nhash, size_t size,
                      unsigned int (*hash_func)(const void *),
                      int (*key_cmp_func)(const void *, const void *),
                      void (*key_destroy_func)(void *),
                      void (*value_destroy_func)(void *))
{
        ...
        *nhash = hash = calloc(1, sizeof (*hash));
        ...
        for ( i = 0; i < hash->lists_size; i++ )
                libidmef_list_init(hash->lists + i);
...
void libidmef_hash_destroy(libidmef_hash_t *hash)
{
        size_t cnt;
        libidmef_list_t *list;
        libidmef_list_t *ptr;
...
int libidmef_hash_add(libidmef_hash_t *hash, void *key, void *value)
{
        hash_elem_t *hash_elem;
        libidmef_list_t *list;
        ...
        list = hash->lists + hash_value_calc(hash, key);
        libidmef_list_add(list, &hash_elem->list);
...
int libidmef_hash_set(libidmef_hash_t *hash, void *key, void *value)
{
        ...
        return libidmef_hash_add(hash, key, value);
}



void *libidmef_hash_get(libidmef_hash_t *hash, const void *key)
{
        hash_elem_t *hash_elem;
...
int libidmef_hash_elem_destroy(libidmef_hash_t *hash, const void *key)
{
        hash_elem_t *hash_elem;
...
void libidmef_hash_iterate(libidmef_hash_t *hash, void (*cb)(void *data))
{
        unsigned int i;
        libidmef_list_t *tmp;
        hash_elem_t *hash_elem;
        ...
        for ( i = 0; i < hash->lists_size; i++ ) {
                libidmef_list_for_each(&hash->lists[i], tmp) {
                        hash_elem = libidmef_list_entry(tmp, hash_elem_t, list);
                        cb(hash_elem->value);
                }
        }
}
