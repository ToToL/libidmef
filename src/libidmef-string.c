...
/*
 * This code include an heavily modified version of the libidmef-strbuf
 * API made by Krzysztof Zaraska, that is now part of libidmef-string.
 */
...
#define LIBIDMEF_ERROR_SOURCE_DEFAULT LIBIDMEF_ERROR_SOURCE_STRING
 ...
 */
#define LIBIDMEF_STRING_OWN_STRUCTURE  0x1

/*
 * String data may be free'd
 */
#define LIBIDMEF_STRING_OWN_DATA       0x2




#if ! defined (LIBIDMEF_VA_COPY)

# if defined (__GNUC__) && defined (__PPC__) && (defined (_CALL_SYSV) || defined (_WIN32))
#  define LIBIDMEF_VA_COPY(ap1, ap2)     (*(ap1) = *(ap2))

# elif defined (LIBIDMEF_VA_COPY_AS_ARRAY)
#  define LIBIDMEF_VA_COPY(ap1, ap2)     memmove ((ap1), (ap2), sizeof(va_list))
...
#define STRING_RETURN_IF_INVALID(str, len) do {                                                                \
        libidmef_return_val_if_fail((len + 1) > len,                                                            \
                                    libidmef_error_verbose(LIBIDMEF_ERROR_INVAL_LENGTH,                          \
                                                          "string length warning: wrap around would occur"));  \
                                                                                                               \
        libidmef_return_val_if_fail(str[len] == 0,                                                              \
                                   libidmef_error_verbose(LIBIDMEF_ERROR_STRING_NOT_NULL_TERMINATED,             \
                                                         "string warning: not nul terminated"));               \
} while(0)



struct libidmef_string {
...
static int string_buf_alloc(libidmef_string_t *string, size_t len)
{
        /*
...
static void string_buf_copy(libidmef_string_t *string, const char *buf, size_t len)
{
...
static int make_string_own(libidmef_string_t *s, size_t len)
{
        char *ptr;
        ...
        s->size = len;
        s->data.rwbuf = ptr;
        s->flags |= LIBIDMEF_STRING_OWN_DATA;
...
static int allocate_more_chunk_if_needed(libidmef_string_t *s, size_t needed_len)
{
        int ret;
        char *ptr;
        size_t len;
        ...
        if ( s->size + len < s->size )
                return libidmef_error(LIBIDMEF_ERROR_INVAL_LENGTH);

        if ( s->flags & LIBIDMEF_STRING_OWN_DATA ) {
                ptr = _libidmef_realloc(s->data.rwbuf, s->size + len);
                if ( ! ptr )
                        return libidmef_error_from_errno(errno);
...
/**
 * libidmef_string_new:
 * @string: Pointer where to store the created #libidmef_string_t.
 *
 * Create a new #libidmef_string_t object, and store in in @string.
 *
...
int libidmef_string_new(libidmef_string_t **string)
{
        *string = calloc(1, sizeof(**string));
        ...
        (*string)->refcount = 1;
        libidmef_list_init(&(*string)->_list);
        (*string)->flags = LIBIDMEF_STRING_OWN_STRUCTURE;
...
/**
 * libidmef_string_ref:
 * @string: Pointer to a #libidmef_string_t object to reference.
 *
...
{
        libidmef_return_val_if_fail(string, NULL);
...
/**
 * libidmef_string_new_dup_fast:
 * @string: Pointer where to store the created #libidmef_string_t object.
 * @str: Initial string value.
 * @len: Lenght of @str.
 *
 * Create a new #libidmef_string_t object with a copy of @str as it's
 * initial value.  The copy is owned by the @string and will be freed
 * upon libidmef_string_destroy().
 *
...
int libidmef_string_new_dup_fast(libidmef_string_t **string, const char *str, size_t len)
{
        ...
        libidmef_return_val_if_fail(str, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        STRING_RETURN_IF_INVALID(str, len);

        ret = libidmef_string_new(string);
        if ( ret < 0 )
                return ret;
        ...
        (*string)->flags |= LIBIDMEF_STRING_OWN_DATA;
...
{
        libidmef_return_val_if_fail(str, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        return libidmef_string_new_dup_fast(string, str, strlen(str));
}



/**
 * libidmef_string_new_nodup_fast:
 * @string: Pointer where to store the created #libidmef_string_t object.
 * @str: Initial string value.
 * @len: Lenght of @str.
 *
 * Create a new #libidmef_string_t object with a reference to @str as
 * initial value.  @str is owned by @string and will be freed upon
 * libidmef_string_destroy().
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
...
{
        libidmef_return_val_if_fail(str, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        return libidmef_string_new_nodup_fast(string, str, strlen(str));
}



/**
 * libidmef_string_new_ref_fast:
 * @string: Pointer where to store the created #libidmef_string_t object.
 * @str: Initial string value.
 * @len: Length of @str.
 *
 * Create a new #libidmef_string_t object with a reference to @str as
 * initial value. @str won't be freed upon libidmef_string_destroy().
 *
 * Returns: 0 on success, a negative value if an error occured.
 */
int libidmef_string_new_ref_fast(libidmef_string_t **string, const char *buf, size_t len)
{
        int ret;
...
{
        libidmef_return_val_if_fail(str, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        return libidmef_string_new_ref_fast(string, str, strlen(str));
}



/**
 * libidmef_string_set_dup_fast:
 * @string: Pointer to a #libidmef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
 * Store a copy of the string pointed by @buf in @string.
 * The @buf copy will be freed upon libidmef_string_destroy().
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
int libidmef_string_set_dup_fast(libidmef_string_t *string, const char *buf, size_t len)
{
        ...
        libidmef_return_val_if_fail(string, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(buf, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        STRING_RETURN_IF_INVALID(buf, len);

        libidmef_string_destroy_internal(string);
...
int libidmef_string_set_dup(libidmef_string_t *string, const char *buf)
{
        libidmef_return_val_if_fail(string, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(buf, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        return libidmef_string_set_dup_fast(string, buf, strlen(buf));
}



/**
 * libidmef_string_set_nodup_fast:
 * @string: Pointer to a #libidmef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
 * Store a reference to the string pointed by @buf in @string.
 * The referenced @buf will be freed upon libidmef_string_destroy().
 *
 * Returns: 0 on success, or a negative value if an error occured.
 */
...
int libidmef_string_set_nodup(libidmef_string_t *string, char *buf)
{
        ...
        return libidmef_string_set_nodup_fast(string, buf, strlen(buf));
}



/**
 * libidmef_string_set_ref_fast:
 * @string: Pointer to a #libidmef_string_t object.
 * @buf: String to store in @string.
 * @len: Lenght of @buf.
 *
...
int libidmef_string_set_ref(libidmef_string_t *string, const char *buf)
{
        ...
        return libidmef_string_set_ref_fast(string, buf, strlen(buf));
}
...
/**
 * libidmef_string_copy_ref:
 * @src: Pointer to a #libidmef_string_t object to copy data from.
 * @dst: Pointer to a #libidmef_string_t object to copy data to.
 *
...
int libidmef_string_copy_ref(const libidmef_string_t *src, libidmef_string_t *dst)
{
        ...
        libidmef_string_destroy_internal(dst);

        dst->size = src->size;
        dst->index = src->index;
        dst->data.robuf = src->data.robuf;
        dst->flags &= ~LIBIDMEF_STRING_OWN_DATA;
...
/**
 * libidmef_string_clone:
 * @src: Pointer to an existing #libidmef_string_t object.
 * @dst: Pointer to an address where to store the created #libidmef_string_t object.
 *
 * Clone @src within a new #libidmef_string_t object stored into @dst.
...
{
        libidmef_return_val_if_fail(string, 0);
        return string->index;
}
...
/**
 * libidmef_string_get_string_or_default:
 * @string: Pointer to a #libidmef_string_t object.
...
/**
 * libidmef_string_get_string_released:
 * @string: Pointer to a #libidmef_string_t object.
...
int libidmef_string_get_string_released(libidmef_string_t *string, char **outptr)
{
        ...
        *outptr = NULL;
        ...
        if ( ! (string->flags & LIBIDMEF_STRING_OWN_DATA) ) {
                *outptr = strdup(string->data.robuf);
                return (*outptr) ? 0 : libidmef_error_from_errno(errno);
        }

        if ( string->index + 1 <= string->index )
                return libidmef_error(LIBIDMEF_ERROR_INVAL_LENGTH);

        *outptr = _libidmef_realloc(string->data.rwbuf, string->index + 1);
        if ( ! *outptr )
                return libidmef_error_from_errno(errno);
...
{
        libidmef_return_val_if_fail(string, TRUE);
...
{
        libidmef_return_if_fail(string);

        if ( (string->flags & LIBIDMEF_STRING_OWN_DATA) && string->data.rwbuf )
...
void libidmef_string_destroy(libidmef_string_t *string)
{
        ...
        if ( ! libidmef_list_is_empty(&string->_list) )
                libidmef_list_del_init(&string->_list);

        libidmef_string_destroy_internal(string);

        if ( string->flags & LIBIDMEF_STRING_OWN_STRUCTURE )
                free(string);
}
...
int libidmef_string_vprintf(libidmef_string_t *string, const char *fmt, va_list ap)
{
        ...
        libidmef_return_val_if_fail(string, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(fmt, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        ...
        ret = libidmef_string_vprintf(string, fmt, bkp);
...
int libidmef_string_sprintf(libidmef_string_t *string, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        ret = libidmef_string_vprintf(string, fmt, ap);
        va_end(ap);
...
/**
 * libidmef_string_cat:
 * @dst: Pointer to a #libidmef_string_t object.
 * @str: Pointer to a string.
 * @len: Length of @str to copy.
 *
 * The libidmef_string_ncat() function appends @len characters from @str to
 * the @dst #libidmef_string_t object over-writing the `\0' character at the
...
int libidmef_string_ncat(libidmef_string_t *dst, const char *str, size_t len)
{
        ...
        if ( dst->flags & LIBIDMEF_STRING_OWN_DATA && len < (dst->size - dst->index) ) {
        ...
        return libidmef_string_ncat(dst, str, len);
}
 ...
 * @str: Pointer to a string.
 *
 * The libidmef_string_cat() function appends the @str string to the @dst
 * libidmef_string_t object over-writing the `\0' character at the end of
...
int libidmef_string_cat(libidmef_string_t *dst, const char *str)
{
        ...
        return libidmef_string_ncat(dst, str, strlen(str));
}
...
int libidmef_string_truncate(libidmef_string_t *string, size_t len)
{
        ...
        libidmef_return_val_if_fail(string, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(len <= string->index, libidmef_error(LIBIDMEF_ERROR_ASSERTION));

        if ( len && ! (string->flags & LIBIDMEF_STRING_OWN_DATA) ) {
...
{
        libidmef_string_truncate(string, 0);
}
...
/**
 * libidmef_string_compare:
 * @str1: Pointer to a #libidmef_string_t object to compare with @str2.
 * @str2: Pointer to a #libidmef_string_t object to compare with @str1.
 *
