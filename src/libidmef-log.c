...
static int do_log_print(libidmef_log_t level, const char *str);


static int debug_level = 0;
static int raise_abort_level = 0;
libidmef_bool_t raise_abort_set = FALSE;
static int log_level = LIBIDMEF_LOG_INFO;

static char *global_prefix = NULL;

static FILE *debug_logfile = NULL;
static libidmef_log_flags_t log_flags = 0;
static int (*global_log_cb)(libidmef_log_t level, const char *str) = do_log_print;
static void (*external_log_cb)(libidmef_log_t level, const char *str) = NULL;
...
static void syslog_win32(int priority, const char *log)
{
        ...
        if ( ! event_source )
                event_source = RegisterEventSource(NULL, TEXT("LibIdmef"));
...
static int do_log_syslog(libidmef_log_t level, const char *str)
{
        int slevel = LOG_INFO;

        if ( level == LIBIDMEF_LOG_CRIT )
                slevel = LOG_CRIT;

        else if ( level == LIBIDMEF_LOG_ERR )
                slevel = LOG_ERR;

        else if ( level == LIBIDMEF_LOG_WARN )
                slevel = LOG_WARNING;

        else if ( level == LIBIDMEF_LOG_INFO )
                slevel = LOG_INFO;

        else if ( level >= LIBIDMEF_LOG_DEBUG )
                slevel = LOG_DEBUG;
...
static inline FILE *get_out_fd(libidmef_log_t level)
{
        return (level < LIBIDMEF_LOG_INFO) ? stderr : stdout;
}
...
static int do_log_external(libidmef_log_t level, const char *str)
{
...
static inline libidmef_bool_t need_to_log(libidmef_log_t level, libidmef_log_t cur)
{
        return (level > cur) ? FALSE : TRUE;
}


static const char *level_to_string(libidmef_log_t level)
{
        if ( level >= LIBIDMEF_LOG_DEBUG )
                return "DEBUG";
        ...
        else if ( level == LIBIDMEF_LOG_CRIT )
                return "CRITICAL";
...
static ssize_t get_header(libidmef_log_t level, char *buf, size_t size)
{
        int ret;
        struct tm *t;
        size_t len = 0;
        ...
        if ( ! (log_flags & LIBIDMEF_LOG_FLAGS_SYSLOG) ) {
                t = localtime(&now);
                if ( t )
...
static void do_log_v(libidmef_log_t level, const char *file,
                     const char *function, int line, const char *fmt, va_list ap)
{
        ...
        if ( level <= LIBIDMEF_LOG_ERR || level >= LIBIDMEF_LOG_DEBUG ) {
        ...
        if ( need_to_log(level, log_level) || need_to_log(level, debug_level) ) {
                ...
                if ( ret < 0 && global_log_cb == do_log_print ) {
                        libidmef_log_set_flags(LIBIDMEF_LOG_FLAGS_SYSLOG);
...
/**
 * _libidmef_log:
 * @level: LIBIDMEF_LOG_PRIORITY_INFO or LIBIDMEF_LOG_PRIORITY_ERROR.
 ...
 * Use the #log macro defined in libidmef-log.h
 */
...
/**
 * libidmef_log_set_flags:
 * @flags:
 */
void libidmef_log_set_flags(libidmef_log_flags_t flags)
{
        if ( flags & LIBIDMEF_LOG_FLAGS_QUIET )
                log_level = LIBIDMEF_LOG_WARN;

        if ( flags & LIBIDMEF_LOG_FLAGS_SYSLOG )
...
{
        debug_level = LIBIDMEF_LOG_DEBUG + level;
}



/**
 * libidmef_log_set_prefix:
 * @prefix: Pointer to the prefix to use.
 *
 * Tell the LibIdmef standard logger to add @prefix before logging
 * a line.
 */
...
/**
 * libidmef_log_set_callback:
 * @log_cb: Callback function.
 *
 * @log_cb() will be called in place of the libidmef function usally
 * used for loging.
 */
...
int _libidmef_log_set_abort_level_from_string(const char *level)
{
        ...
        struct {
                const char *prefix;
                libidmef_log_t level;
        } tbl[] = {
                { "CRIT",  LIBIDMEF_LOG_CRIT  },
                { "ERR",   LIBIDMEF_LOG_ERR   },
                { "WARN",  LIBIDMEF_LOG_WARN  },
                { "INFO",  LIBIDMEF_LOG_INFO  },
                { "DEBUG", LIBIDMEF_LOG_DEBUG }
        };

        libidmef_return_val_if_fail(level != NULL, libidmef_error(LIBIDMEF_ERROR_ASSERTION));

        for ( i = 0; i < sizeof(tbl) / sizeof(*tbl); i++ ) {
                if ( strncasecmp(tbl[i].prefix, level, strlen(tbl[i].prefix)) == 0 ) {
                        _libidmef_log_set_abort_level(tbl[i].level);
                        return 0;
                }
        }
        ...
        if ( eptr != (level + strlen(level)) || lvalue == LONG_MIN || lvalue == LONG_MAX ) {
                libidmef_log(LIBIDMEF_LOG_WARN, "Invalid abort level specified: '%s'.\n", level);
                return -1;
        }

        _libidmef_log_set_abort_level(lvalue);
...
void libidmef_log(libidmef_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libidmef_log_v(level, "", "", 0, fmt, ap);
        va_end(ap);
}
...
void libidmef_log_debug(libidmef_log_t level, const char *fmt, ...)
{
        ...
        va_start(ap, fmt);
        _libidmef_log_v(LIBIDMEF_LOG_DEBUG + level, "", "", 0, fmt, ap);
        va_end(ap);
}
