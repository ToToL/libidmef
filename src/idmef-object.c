...
idmef_object_t *idmef_object_ref(idmef_object_t *obj)
{
        ...
        libidmef_return_val_if_fail(ret == 0, NULL);
...
{
        return idmef_class_print_binary(obj->_idmef_object_id, obj, fd);
}

void idmef_object_add(libidmef_list_t *head, idmef_object_t *object)
{
        libidmef_return_if_fail(idmef_class_is_listed(object->_idmef_object_id));
        libidmef_list_add(head, &((idmef_linked_object_t *) object)->_list);
}


void idmef_object_add_tail(libidmef_list_t *head, idmef_object_t *object)
{
        libidmef_return_if_fail(idmef_class_is_listed(object->_idmef_object_id));
        libidmef_list_add_tail(head, &((idmef_linked_object_t *) object)->_list);
}
...
{
        libidmef_return_if_fail(idmef_class_is_listed(object->_idmef_object_id));
        libidmef_list_del(&((idmef_linked_object_t *) object)->_list);
}
...
void idmef_object_del_init(idmef_object_t *object)
{
        libidmef_return_if_fail(idmef_class_is_listed(object->_idmef_object_id));
        libidmef_list_del(&((idmef_linked_object_t *) object)->_list);
        libidmef_list_init(&((idmef_linked_object_t *) object)->_list);
}


void *idmef_object_get_list_entry(libidmef_list_t *elem)
{
        return libidmef_list_entry(elem, idmef_linked_object_t, _list);
}
