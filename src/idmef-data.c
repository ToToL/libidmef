...
{
        libidmef_return_val_if_fail(data, NULL);
...
{
        libidmef_return_val_if_fail(data, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
{
        libidmef_return_val_if_fail(data, 0);
        return data->len;
}
...
{
        libidmef_return_val_if_fail(data, TRUE);
        return (data->len == 0) ? TRUE : FALSE;
}



static int bytes_to_string(libidmef_string_t *out, const unsigned char *src, size_t size)
{
        char c;
        int ret;
        ...
        while ( size ) {
                ret = libidmef_string_ncat(out, &b64tbl[(src[0] >> 2) & 0x3f], 1);
                if ( ret < 0 )
                        return ret;

                c = b64tbl[((src[0] << 4) + ((--size) ? src[1] >> 4 : 0)) & 0x3f];

                ret = libidmef_string_ncat(out, &c, 1);
                if ( ret < 0 )
                        return ret;
 ...
 * @data: Pointer to an #idmef_data_t object.
 * @out: Pointer to a #libidmef_string_t to store the formated data into.
 *
...
int idmef_data_to_string(const idmef_data_t *data, libidmef_string_t *out)
{
        ...
        case IDMEF_DATA_TYPE_CHAR:
                ret = libidmef_string_sprintf(out, "%c", data->data.char_data);
                break;
        ...
        case IDMEF_DATA_TYPE_BYTE:
                 ...
                 */
                ret = libidmef_string_sprintf(out, "%u", (unsigned int) data->data.byte_data);
                break;

        case IDMEF_DATA_TYPE_UINT32:
        case IDMEF_DATA_TYPE_INT:
                ret = libidmef_string_sprintf(out, "%" LIBIDMEF_PRId64, data->data.int_data);
                break;

        case IDMEF_DATA_TYPE_FLOAT:
                ret = libidmef_string_sprintf(out, "%f", data->data.float_data);
                break;

        case IDMEF_DATA_TYPE_CHAR_STRING:
                ret = libidmef_string_sprintf(out, "%s", (const char *) data->data.ro_data);
                break;
