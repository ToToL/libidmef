...
static void print_indent(libidmef_io_t *fd)
{
        int cnt;

        for ( cnt = 0; cnt < indent; cnt++ )
                libidmef_io_write(fd, " ", 1);
}



static void print_string(libidmef_string_t *string, libidmef_io_t *fd)
{
        if ( libidmef_string_is_empty(string) )
                libidmef_io_write(fd, "<empty>", 7);
        else
                libidmef_io_write(fd, libidmef_string_get_string(string), libidmef_string_get_len(string));
}



static void print_uint8(uint8_t i, libidmef_io_t *fd)
{
        int len;
        char buf[sizeof("255")];
        ...
        libidmef_io_write(fd, buf, len);
}


static void print_uint16(uint16_t i, libidmef_io_t *fd)
{
        int len;
...
static void print_int32(int32_t i, libidmef_io_t *fd)
{
        int len;
...
static void print_uint32(uint32_t i, libidmef_io_t *fd)
{
        int len;
        char buf[sizeof("4294967296")];
...
static void print_uint64(uint64_t i, libidmef_io_t *fd)
{
        int len;
        char buf[sizeof("18446744073709551616")];

        len = snprintf(buf, sizeof(buf), "%" LIBIDMEF_PRIu64, i);
        libidmef_io_write(fd, buf, len);
}



static void print_float(float f, libidmef_io_t *fd)
{
        int len;
        char buf[32];
...
static void print_time(idmef_time_t *t, libidmef_io_t *fd)
{
        int len;
        time_t _time;
        struct tm _tm;
...
static int print_data(idmef_data_t *data, libidmef_io_t *fd)
{
        int ret;
        ...
        libidmef_io_write(fd, libidmef_string_get_string(out), libidmef_string_get_len(out));
        libidmef_string_destroy(out);
...
static void print_enum(const char *s, int i, libidmef_io_t *fd)
{
        int len;
        char buf[512];
 ...
 * @fd: Pointer to a #libidmef_io_t object where to print @ptr to.
 *
 * This function will convert @ptr to a string suitable for writing,
 * and write it to the provided @fd descriptor.
 */
void idmef_additional_data_print(idmef_additional_data_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                if ( field ) {
                        print_indent(fd);
                        libidmef_io_write(fd, tmp, sizeof(tmp) - 1);
                        print_string(field, fd);
                        libidmef_io_write(fd, "\n", sizeof("\n") - 1);
                }
        }

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "type: ", sizeof("type: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_reference_print(idmef_reference_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "origin: ", sizeof("origin: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_classification_print(idmef_classification_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_user_id_print(idmef_user_id_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_user_print(idmef_user_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "category: ", sizeof("category: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_address_print(idmef_address_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_process_print(idmef_process_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_web_service_print(idmef_web_service_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_snmp_service_print(idmef_snmp_service_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_service_print(idmef_service_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_node_print(idmef_node_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_source_print(idmef_source_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "spoofed: ", sizeof("spoofed: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_file_access_print(idmef_file_access_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_inode_print(idmef_inode_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_checksum_print(idmef_checksum_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "algorithm: ", sizeof("algorithm: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_file_print(idmef_file_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "fstype: ", sizeof("fstype: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_linkage_print(idmef_linkage_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_target_print(idmef_target_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "decoy: ", sizeof("decoy: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_analyzer_print(idmef_analyzer_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_alertident_print(idmef_alertident_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_impact_print(idmef_impact_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "severity: ", sizeof("severity: ") - 1);
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "completion: ", sizeof("completion: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_action_print(idmef_action_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_confidence_print(idmef_confidence_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                {
                        print_indent(fd);
                        libidmef_io_write(fd, "rating: ", sizeof("rating: ") - 1);
        ...
        print_indent(fd);
        libidmef_io_write(fd, "confidence: ", sizeof("confidence: ") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_assessment_print(idmef_assessment_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                if ( field ) {
                        print_indent(fd);
                        libidmef_io_write(fd, "confidence:\n", sizeof("confidence:\n") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_tool_alert_print(idmef_tool_alert_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_correlation_alert_print(idmef_correlation_alert_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_overflow_alert_print(idmef_overflow_alert_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_alert_print(idmef_alert_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
...

        {
                ...
                if ( field ) {
                        print_indent(fd);
                        libidmef_io_write(fd, "classification:\n", sizeof("classification:\n") - 1);
...

        {
                ...
                if ( field ) {
                        print_indent(fd);
                        libidmef_io_write(fd, "assessment:\n", sizeof("assessment:\n") - 1);
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_heartbeat_print(idmef_heartbeat_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
 ...
 * and write it to the provided @fd descriptor.
 */
void idmef_message_print(idmef_message_t *ptr, libidmef_io_t *fd)
{
        if ( ! ptr )
                return;
