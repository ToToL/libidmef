...
*         Yoann Vandoorselaere <yoann@libidmef-ids.com>
*
...
#define LIBIDMEF_ERROR_SOURCE_DEFAULT LIBIDMEF_ERROR_SOURCE_IDMEF_PATH
...
static libidmef_bool_t flush_cache = FALSE;
static libidmef_hash_t *cached_path = NULL;
...
static int initialize_path_cache_if_needed(void)
{
        ...
        return libidmef_hash_new2(&cached_path, HASH_DEFAULT_SIZE, path_hash_func, path_key_cmp_func, NULL, flush_cache_if_wanted);
}
...
static int build_name(idmef_path_t *path)
{
        ...
        for ( i = 0; i < path->depth; i++ ) {
                ...
                if ( ! name )
                        return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_INTEGRITY);
...
static inline libidmef_bool_t has_index_key(libidmef_list_t *elem, const char *index_key)
{
        idmef_key_listed_object_t *obj = libidmef_list_entry(elem, idmef_key_listed_object_t, list);
        return (obj->objkey && strcmp(libidmef_string_get_string_or_default(obj->objkey, ""), index_key) == 0) ? TRUE : FALSE;
}
...
static int set_index_key(const idmef_path_element_t *elem, void *ptr)
{
        ...
        if ( obj->objkey )
                libidmef_string_destroy(obj->objkey);

        return libidmef_string_new_dup(&obj->objkey, elem->index_key);
}
...
static int idmef_path_get_list_internal(idmef_value_t **value_list,
                                        ...
                                        libidmef_list_t *list, idmef_class_id_t parent_class)
{
        int ret;
        libidmef_list_t *tmp;
        idmef_value_t *value;
        ...
        libidmef_list_for_each(list, tmp) {
                value = NULL;
...
static int idmef_path_get_nth_internal(idmef_value_t **value, const idmef_path_t *path,
                                       unsigned int depth, libidmef_list_t *list,
                                       idmef_class_id_t parent_class, int which, const char *index_key)
{
        int cnt = 0;
        libidmef_list_t *tmp = NULL;
        ...
        if ( which >= 0 ) {
                ...
                libidmef_list_for_each_reversed(list, tmp) {
                        if ( cnt++ == which )
                                return idmef_path_get_internal(value, path, depth, idmef_linked_object_get_object(tmp), parent_class);
                }
        }
...
static void delete_listed_child(void *parent, idmef_class_id_t class, const idmef_path_element_t *elem)
{
        int ret;
        void *obj;
        libidmef_list_t *head, *tmp, *bkp;

        ret = idmef_class_get_child(parent, class, elem->position, (void *) &head);
        if ( ret < 0 )
                return;

        libidmef_list_for_each_safe(head, tmp, bkp) {
                 ...
                 *
                 * We manually call libidmef_list_del_init() in order to
                           ...
                           int index_override, void *ptr, idmef_value_t *value)
{
        void *prevptr = NULL;
        int ret, index, j;
        libidmef_bool_t is_last_element;
...
int idmef_path_get(const idmef_path_t *path, void *obj, idmef_value_t **ret)
{
        ...
        if ( object->_idmef_object_id != path->top_class )
                return libidmef_error_verbose(LIBIDMEF_ERROR_GENERIC, "path for object '%s' used with '%s' object root",
...
int idmef_path_set(const idmef_path_t *path, void *obj, idmef_value_t *value)
{
        ...
        if ( path->depth < 1 )
                return libidmef_error_verbose(LIBIDMEF_ERROR_GENERIC, "Path with depth of 0 are not allowed");
...
static int copy_path_name(idmef_path_t *path, const char *buffer)
{
        ...
        if ( len >= sizeof(path->name) )
                return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_LENGTH);
...
static int idmef_path_create(idmef_path_t **path, idmef_class_id_t rootclass, const char *buffer)
{
        ...
        *path = libidmef_hash_get(cached_path, &tmp);
        gl_lock_unlock(cached_path_mutex);
...
static int idmef_path_parse_new(idmef_path_t *path, idmef_class_id_t rootclass, const char *buffer)
{
        ...
        do {
                ...
                if ( ptr2 ) {
                        ...
                        if ( ! end )
                                return libidmef_error_verbose(LIBIDMEF_ERROR_GENERIC, "Malformed IDMEFPath index : missing ')' character");
                        ...
                        *end = *ptr2 = '\0';
                        if ( strcmp(tok, "<<") == 0 )
                                ...
                                if ( index == 0 && errptr == tok )
                                        return libidmef_error_verbose(LIBIDMEF_ERROR_GENERIC, "Invalid index specified : `%s`", tok);
                        }
                        *end = ')';
                }
                ...
                if ( index == INDEX_UNDEFINED )
                        ...
                        if ( ! idmef_class_is_child_list(class, child) )
                                return libidmef_error_verbose(LIBIDMEF_ERROR_IDMEF_PATH_INDEX_FORBIDDEN,
...
int idmef_path_new_from_root_fast(idmef_path_t **path, idmef_class_id_t rootclass, const char *buffer)
{
        ...
        libidmef_return_val_if_fail(buffer, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        ...
        if ( libidmef_hash_add(cached_path, *path, *path) < 0 ) {
...
int idmef_path_new_v(idmef_path_t **path, const char *format, va_list args)
{
        ...
        libidmef_return_val_if_fail(format, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
{
        libidmef_return_val_if_fail(path, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
static inline int invalidate(idmef_path_t *path)
{
        ...
        if ( path->refcount == 2 ) {
                gl_lock_lock(cached_path_mutex);
                ret = libidmef_hash_elem_destroy(cached_path, path);
                gl_lock_unlock(cached_path_mutex);
...
int idmef_path_set_index(idmef_path_t *path, unsigned int depth, int index)
{
        ...
        libidmef_return_val_if_fail(path, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(depth < path->depth, libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_DEPTH));

        if ( index == INDEX_FORBIDDEN || index == INDEX_KEY )
                return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_INDEX_RESERVED);

        if ( path->elem[depth].index == INDEX_FORBIDDEN )
                return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_INDEX_FORBIDDEN);
...
int idmef_path_get_index(const idmef_path_t *path, unsigned int depth)
{
        ...
        if ( path->elem[depth].index == INDEX_UNDEFINED )
                return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_INDEX_UNDEFINED);
...
int idmef_path_make_child(idmef_path_t *path, const char *child_name, int index)
{
        ...
        libidmef_return_val_if_fail(path, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(child_name, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
int idmef_path_make_parent(idmef_path_t *path)
{
        ...
        if ( path->depth == 0 )
                return libidmef_error(LIBIDMEF_ERROR_IDMEF_PATH_PARENT_ROOT);
...
int idmef_path_ncompare(const idmef_path_t *p1, const idmef_path_t *p2, unsigned int depth)
{
        ...
        libidmef_return_val_if_fail(p1, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
        libidmef_return_val_if_fail(p2, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
{
        libidmef_return_val_if_fail(path && result, libidmef_error(LIBIDMEF_ERROR_ASSERTION));
...
{
        libidmef_return_val_if_fail(path, NULL);
 ...
 */
libidmef_bool_t idmef_path_is_ambiguous(const idmef_path_t *path)
{
        size_t i;

        libidmef_return_val_if_fail(path, FALSE);
...
int idmef_path_has_lists(const idmef_path_t *path)
{
        ...
        libidmef_return_val_if_fail(path, 0);
...
libidmef_bool_t idmef_path_is_list(const idmef_path_t *path, int depth)
{
        libidmef_return_val_if_fail(path, FALSE);
...
const char *idmef_path_get_name(const idmef_path_t *path, int depth)
{
        ...
        libidmef_return_val_if_fail(path, NULL);
        libidmef_return_val_if_fail(depth < 0 || (size_t) depth < path->depth, NULL);
...
void _idmef_path_cache_lock(void)
{
        ...
        if ( cached_path )
                libidmef_hash_iterate(cached_path, path_lock_cb);
}
...
void _idmef_path_cache_reinit(void)
{
        ...
        if ( cached_path )
                libidmef_hash_iterate(cached_path, path_reinit_cb);
}
...
{
        if ( cached_path )
                libidmef_hash_iterate(cached_path, path_unlock_cb);
...
void _idmef_path_cache_destroy(void)
{
        ...
        flush_cache = TRUE;
        libidmef_hash_destroy(cached_path);
        flush_cache = FALSE;
}
