...
SV *IDMEFValueList_to_SWIG(TARGET_LANGUAGE_SELF self, const LibIdmef::IDMEFValue &value, void *extra)
{
        int j = 0, ret;
...
static void _cb_perl_log(int level, const char *str)
{
        ...
        perl_call_sv(__libidmef_log_func, G_VOID);
...
static int _cb_perl_write(libidmef_msgbuf_t *fd, libidmef_msg_t *msg)
{
        ssize_t ret;
        PerlIO *io = (PerlIO *) libidmef_msgbuf_get_data(fd);

        ret = PerlIO_write(io, (const char *) libidmef_msg_get_message_data(msg), libidmef_msg_get_len(msg));
...
static ssize_t _cb_perl_read(libidmef_io_t *fd, void *buf, size_t size)
{
        int ret;
        PerlIO *io = (PerlIO *) libidmef_io_get_fdptr(fd);
...
%typemap(in, fragment="TransitionFunc") void (*log_cb)(int level, const char *log) {
        if ( __libidmef_log_func )
                SvREFCNT_dec(__libidmef_log_func);
